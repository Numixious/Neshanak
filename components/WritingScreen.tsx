      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        {imageData && !isLoadingImage && (
          <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm p-6 rounded-2xl shadow-xl flex flex-col gap-4">
            ...
            {feedback && (
              <div className="relative mt-4 p-4 bg-green-100 dark:bg-green-900/50 border-r-4 border-green-500 dark:border-green-600 text-green-800 dark:text-green-200 rounded-lg cursor-pointer" onClick={() => speak(feedback)}>
                <p className="text-xl/relaxed font-medium">{feedback}</p>
                 <button
                    onClick={(e) => { e.stopPropagation(); speak(feedback); }}
                    className="absolute top-1/2 left-2 -translate-y-1/2 p-2 text-green-600 dark:text-green-300 opacity-70 hover:opacity-100"
                    aria-label="بلند خواندن بازخورد"
                >
