...
const WritingSignsMcqScreen: React.FC<WritingSignsMcqScreenProps> = ({ navigateTo, navigateBack }) => {
    ...
    const playSound = async (text: string, isAutoplay: boolean = false) => {
        if (audioRef.current) {
            audioRef.current.pause();
        }
        setIsSpeaking(true);
        if (isAutoplay) setAutoplayFailed(false);
        try {
            ...
            console.error("Could not fetch or play audio:", error);
            setIsSpeaking(false);
    ...
    useEffect(() => {
        if (currentQuestion) {
            playSound(currentQuestion.soundToSpeak, true);
        }
        return () => {
            if (audioRef.current) {
                audioRef.current.pause();
    ...
    return (
        <div className="w-full max-w-2xl mx-auto p-4 md:p-8 animate-jump-in">
            ...
            <div className="bg-white dark:bg-slate-800 p-6 md:p-10 rounded-3xl shadow-2xl">
                {autoplayFailed && (
                    <div className="text-center text-md font-semibold text-amber-700 dark:text-amber-400 mb-4 p-3 bg-amber-100 dark:bg-amber-900/50 rounded-lg animate-jump-in">
                        <p>برای شنیدن صدا، روی دکمه بلندگو کلیک کن.</p>
                    </div>
                )}
                <button
