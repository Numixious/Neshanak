      ...
      <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm p-6 md:p-8 rounded-2xl shadow-xl min-h-[300px] flex items-center justify-center">
        {isLoading ? (
          ...
          <div className="relative">
            <p className="text-2xl/loose md:text-3xl/loose text-brand-text dark:text-slate-200 font-medium text-right whitespace-pre-wrap cursor-pointer" onClick={() => speak(story)}>
              {story}
            </p>
            <button
                onClick={() => speak(story)}
                className="absolute top-0 -left-4 p-2 text-brand-secondary opacity-70 hover:opacity-100 transition-opacity"
                aria-label="بلند خواندن داستان"
            >
